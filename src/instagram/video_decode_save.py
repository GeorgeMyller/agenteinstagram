    ...
    def process(video_base64):
        ...
        try:
            ...
            with open(filepath, "wb") as f:
                f.write(video_data)
            logger.info(f"Vídeo base64 salvo em: {filepath}")
            return filepath
        except Exception as e:
            logger.exception(f"Erro ao processar vídeo base64: {str(e)}")
            raise
            ...
            for file in os.listdir(temp_dir):
                if file.startswith("temp-") and file.endswith(".mp4"):
                    ...
                    if file_age > max_age_seconds:
                        os.remove(file_path)
                        logger.info(f"Removido vídeo antigo: {file_path}")
        except Exception as e:
            logger.error(f"Erro ao limpar vídeos antigos: {str(e)}")